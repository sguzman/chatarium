...
4. observe a visible window in the retained Edge Job Object and treat its close as the one completion action;
5. wait up to four seconds for all job members to exit naturally, then terminate only that job and verify it is empty; release and verify the harness lock;
6. only after the job is empty and the lock is released, reopen the same persistent profile over the Windows anonymous pipe;
...
The dedicated profile remains on disk for later runs. Phase 1 assigns the suspended Edge root process to a private Job Object before it can start, so descendants stay in a retained ownership boundary if the root exits or they are reparented. Visible top-level windows are inspected through native Win32 APIs; owner PIDs are checked against the exact process object with `IsProcessInJob`, preventing PID reuse from being treated as ownership. The window-close grace is 500 ms to allow a just-closed window to reappear, followed by up to four seconds for natural job exit and a bounded termination verification. Forced termination targets the private job and happens only after the owned visible window has been observed closed. If window or job ownership cannot be established, `init` leaves the browser and profile lock intact rather than risking an active login session. Harness locks record a versioned process creation identity; an orphaned legacy or current lock is reclaimed automatically only under a bounded recovery mutex after the exact-profile Chromium ProcessSingleton is confirmed absent. Chromium-owned lock files and profile contents are never removed. Authentication is not programmatically verified: a matching target is operator-completed bootstrap evidence only. This command must not import credentials from another profile, extract credentials or cookies, inspect browser storage or authentication traffic, automate login, or bypass any authentication flow. Phase 2 uses the existing pipe transport only and has no TCP fallback. Failures preserve the profile; the pipe phase never starts unless phase 1's job is empty and the harness lock is absent.
